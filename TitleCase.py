...
# letter of the word is in upper case) or 
# (b) considered to be an exception and put entirely into lower 
...
# the case of the minor words string -- it should behave 
...
def title_case(title, minor_words = None):
        words = [ele.capitalize() for ele in title.lower().split(' ')]
        if minor_words == None:
            return ' '.join(words) 

        minor_words = [ele.capitalize() for ele in minor_words.lower().split(' ')]
        for ele in words[1:]:
            if ele in minor_words:
                words[words[1:].index(ele) + 1] = ele.lower()
        return ' '.join(words)    


print(title_case(input, exceptions))
